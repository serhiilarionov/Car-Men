...
{
    use MakeCategoryTrait, MakeCompanyTrait, ApiTestTrait, DatabaseTransactions;
    ...
    /**
     * @test read companies by category id
     */
    public function testReadCompanies()
    {
        ...
        $dbCategory = $this->categoryRepo->with('companies')->find($category->id);
        $dbCompany = $dbCategory->companies->first();

        $dbCategory = $dbCategory->toArray();
        $this->assertModelData($category->toArray(), $dbCategory);
        $this->assertModelData($company->toArray(), $dbCompany->toArray());
