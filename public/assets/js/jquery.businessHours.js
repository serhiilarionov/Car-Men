...
(function($) {
  $.fn.businessHours = function(opts) {
    var defaults = {
      ...
      defaultOperationLunchTimeFrom: null,
      defaultOperationLunchTimeTill: null,
      defaultActive: true,
      //labelOn: "Working day",
      //labelOff: "Day off",
      //labelTimeFrom: "from:",
    ...
    var methods = {
          ...
          serialize: function() {
            ...
            container.find(".operationState").each(function(num, item) {
              ...
              data.push({
                ...
                lunchTimeFrom: isWorkingDay ? dayContainer.find("[name='lunchStartTime']").val() : null,
                lunchTimeTill: isWorkingDay ? dayContainer.find("[name='lunchEndTime']").val() : null
              });
            });
      ...
      initView: function(options) {
        ...
        $.each(options.weekdays, function(pos, weekday) {
          ...
          operationDayNode.attr( 'id', pos );

          var isWorkingDay = $this.getValueOrDefault(day && day.isActive, options.defaultActive);
          operationDayNode.find('.operationState').prop('checked', isWorkingDay);

          var timeFrom = $this.getValueOrDefault(day && day.timeFrom, options.defaultOperationTimeFrom);
          initTimeBox(operationDayNode.find('[name="startTime"]'), timeFrom, options.inputDisabled);

          var endTime = $this.getValueOrDefault(day && day.timeTill, options.defaultOperationTimeTill);
          initTimeBox(operationDayNode.find('[name="endTime"]'), endTime, options.inputDisabled);

          var lunchTimeFrom = $this.getValueOrDefault(day && day.lunchTimeFrom, options.defaultOperationLunchTimeFrom);
          initTimeBox(operationDayNode.find('[name="lunchStartTime"]'), lunchTimeFrom, options.inputDisabled);

          var lunchEndTime = $this.getValueOrDefault(day && day.lunchTimeTill, options.defaultOperationLunchTimeTill);
          initTimeBox(operationDayNode.find('[name="lunchEndTime"]'), lunchEndTime, options.inputDisabled);
        });
